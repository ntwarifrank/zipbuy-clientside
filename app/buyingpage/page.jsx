...
const BuyingPage = () => {
  ...
  const CategoryButton = ({ category, isActive }) => (
    <div 
      ...
      className={`px-4 py-1 rounded-full max-w-fit text-sm cursor-pointer ${
        mode ? isActive 
            ? 'bg-black text-white' 
  ...
  const ProductCard = ({ product, index }) => (
    <div
      key={index}
      className={mode ? 
        `card ${dashboardState ? 'w-[24%]' : 'w-[23%]'} h-[280px] overflow-hidden` : 
        `shadow-gray-800 shadow-sm rounded-lg card ${dashboardState ? 'w-[24%]' : 'w-[23%]'} h-[280px] overflow-hidden`}
    >
      ...
      <div className="py-1 px-3 h-[40%] flex flex-col gap-1">
        <div className={mode ? 
          'text-md h-[50%] py-1 font-semibold flex flex-wrap text-black' : 
          'text-darkText text-md h-[50%] py-1 font-semibold flex flex-wrap'}>
        ...
        {Array.isArray(searchedProduct) && searchedProduct.length > 0 && search ? (
          <div className="w-full">
            ...
            <div className="w-full flex flex-row mt-0 py-2">
              <div onClick={resetSearch} className={`${mode ? '' : 'bg-gray-800 text-gray-400'}  py-1 rounded-full max-w-fit mr-2 text-sm cursor-pointer`}>
            ...
            <div className="card-container flex flex-row flex-wrap gap-3 px-3 py-3">
